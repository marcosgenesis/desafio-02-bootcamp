...
class SubscriptionController{
  ...
  async store(req,res){
    ...
    */
    ...
    const alrealdySub = await Subscription.findOne({where:{
      user_id:req.userId,
      meetup_id:meetup.id
    },include:[{
      model:Meetup,
    ...
    if (alrealdySub) {
      ...
      return res.status(400).json({error:'You already subscribed in this meetup'
    })}
   const organizer = await User.findByPk(meetup.user_id)   
   const user = await User.findByPk(req.userId)  
   const sub = await Subscription.create({user_id:req.userId,meetup_id:meetup.id})
