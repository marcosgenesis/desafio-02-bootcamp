...
class MeetupsController{
  async index(req,res){ 
      ...
      console.log(meetups)
    if(!meetups) return res.status(400).json({
  ...
  async store(req,res){
     ...
     * Check past dates
     */
    if (isBefore(parsedDate,new Date())) return res.status(400).json({
      error: 'You cant create a meetup with past dates'
    })
    const isBanner = await File.findOne({
      where:{id:banner_id}
    })
    if(!isBanner) return res.status(400).json({error:'File does not exists'})
  ...
  async update(req,res){
    ...
    if(!meetup) return res.status(400).json({
      error:'Meetup not found'
    })
    if (meetup.user_id !== user_id) return res.status(401).json({
      error:'You are not organizator of this meetup'
    })
    /**
     * Check past dates of meetups
     */
    if(isBefore(meetup.date, new Date())) return res.status(400).json({
      error: "you can't change past dates"
    })  
    if(isBefore(parseISO(req.body.date),new Date())) return res.status(400).json({
      error: "you can't  change past dates"
    })
    const {title,location,description,date} = await meetup.update(req.body)
    return res.json({
  ...
  async delete(req,res){
    ...
    if (isBefore(meetup.date,new Date())) return res.status(401).json({
